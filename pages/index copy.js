...
import vendors from "../data/vendors";              // ← همان قبلی
import services from "../data/services.json";       // ← خدمات از فایل JSON

// رنگ‌ها و کمک‌تابع‌ها (همان قبلی)
const TEAL = "#14b8a6";
const YELLOW = "#f4c21f";
...
// دکمه‌های هیرو (بدون تغییر منطقی)
...
/* ---------- کارت برند «تجهیزات» (فقط لوگو؛ چپ چین) ---------- */
    ...
    <Link href={link} className="group block">
      ...
      >
        <picture className="pointer-events-none select-none absolute inset-0">
          ...
          <img src={artPng} alt="" aria-hidden="true"
        ...
        {/* لوگو در سمت چپ (در RTL با justify-end) */}
        <div className="relative flex items-center ltr:justify-start rtl:justify-end">
          <div className="w-14 h-14 shrink-0 rounded-xl bg-white ring-1 ring-black/5 shadow-sm grid place-items-center transition-transform duration-200 group-hover:scale-[1.03] overflow-hidden">
            <picture>
              ...
              <img
                src={png} alt={title} width={56} height={56}
...
/* ---------- کارت «خدمات» با پس‌زمینه‌ی ۷۰٪ از رنگ‌های برند ---------- */
